        ...
        <div class="func-row row" >
            <div class="col-xs-12">
                <div class="form-inline pull-left">
                    <el-input v-model="searchCond" @blur="validateSearchCond" placeholder="格式如：policy=policy[;worker=worker]" class="serach my-el-button-left"></el-input>
            ...
            <div class="data-zone">
                <el-table
                    ...
                    <el-table-column
                            ...
                            width="180rem">
                        <template scope="scope">
                            <span v-if="scope.row.state && scope.row.state.state == 'success'" style="color: green">WAITING</span>
                            <span v-else-if="scope.row.state && scope.row.state.state == 'start'" style="color: blue">START</span>
                            <span v-else-if="scope.row.state && scope.row.state.state == 'failed'" style="color: red">FAILED</span>
                            <el-progress v-else-if="scope.row.state" :percentage="parseInt(scope.row.state.state)"></el-progress>
                            <span v-else style="color: #F7AD01">NOT START</span>
                        </template>
                    </el-table-column>
        ...
        <div class="elrement-ui-dialog-modul">
            <el-dialog :title="taskFormTitle" :visible.sync="dialogNewTaskFormVisible">
                <el-form :model="taskForm" :rules="taskRules" ref="taskForm" label-width="120px" class="dremo-ruleForm">
                    ...
                    <el-form-item label="关联策略" prop="policy_id" required>
                    ...
                    <el-form-item label="备份目的地" prop="destination">
                        <el-input type="text" v-model="taskForm.destination" placeholder="格式：卷名[IP]:目录名, 目录名以 / 分割，目录名可以包含字母、数值及下划线以“/”结束，，如 glusterfs:/A/"></el-input>
                    </el-form-item>
            ...
            <el-dialog title="任务详情" :visible.sync="dialogTaskDetailFormVisible">
                <div class="detail">
                    ...
                    <div class="detail-item">
                        <div class="item-key">备份保留时间&nbsp:</div>
                        <div class="item-value">&nbsp{{ taskDetailForm.task.duration }}</div>
                    </div>
                    <div class="detail-item">
                        <div class="item-key">备份策略名&nbsp:</div>
                        <div class="item-value">&nbsp{{ taskDetailForm.policy.name }}</div>
                    </div>
                    <div class="detail-item">
                        <div class="item-key">备份策略详情&nbsp:</div>
                        <div class="item-value">&nbsp{{ taskDetailForm.policy.description }}</div>
                    </div>
                    <div class="detail-item">
                        <div class="item-key">备份开始时间&nbsp:</div>
                        <div class="item-value">&nbsp{{ taskDetailForm.policy.start_time | timeStamp2datetime }}</div>
                    </div>
                    <div class="detail-item">
                        <div class="item-key">备份周期&nbsp:</div>
                        <div class="item-value">&nbsp{{ taskDetailForm.policy.interval }}</div>
                    </div>
                    <div class="detail-item">
            ...
            <el-dialog :title="currTaskStateTabTitle" :visible.sync="dialogTaskStateDetailTableVisible">
                <el-table :data="taskStates"
                    ...
                    <el-table-column label="任务状态">
                        <template scope="scope">
                            <span v-if="scope.row && scope.row.state == 'success'" style="color: green">WAITING</span>
                            <span v-else-if="scope.row && scope.row.state == 'start'" style="color: blue">START</span>
                            <span v-else-if="scope.row && scope.row.state == 'failed'" style="color: red">FAILED</span>
...
export default {
    data() {
        ...
        return {
            ...
            taskRules: {
                name: [
                    { required: true, message: '请输入具有意义任务名称', trigger: 'blur' },
                ...
                policy_id: [
                    //{required: true, message: '请为任务选择执行策略', trigger: 'blur'}
                ],
                worker_id: [
                    //{required: true, message: '请选择执行任务的工作机', trigger: 'blur'}
                ],
                source:[
                    {required: true, validator: validateBackupSAddr, trigger: 'blur'}
                ],
                destination: [
                    {required: true, validator: validateBackupDAddr, trigger: 'blur'}
                ],
                saetime: [
                    {required: true, validator: validateSaeTime, trigger: 'blur'}
                ]
            }
        }
    },
